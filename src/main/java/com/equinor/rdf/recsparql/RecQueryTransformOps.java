...
public class RecQueryTransformOps {
    /** Transform a query based on a mapping from {@link Var} variable to replacement {@link Node}. */
    public static RecQuery transform(RecQuery query, Map<Var, ? extends Node> substitutions) {
     ...
     */
    public static RecQuery transformQuery(RecQuery query, Map<String, ? extends RDFNode> substitutions) {
     ...
     */
    public static RecQuery transform(RecQuery query, ElementTransform transform, ExprTransform exprTransform) {
        RecQuery q2 = RecQueryTransformOps.shallowCopy(query);
    ...
    public static RecQuery transform(RecQuery query, ElementTransform transform) {
    ...
    static class RecQueryShallowCopy implements QueryVisitor {
        final RecQuery newQuery = new RecQuery();
        ...
        public void startVisit(Query query) {
            ...
            if(query instanceof RecQuery){
                ...
                var stack = new Stack<RecQuery.RecClause>();
                for(var sq:rq.getRecursiveQueries()){
                    if(!newQuery.isRecursive) newQuery.setRecursive(query.getSyntax());
                    stack.push(sq);
                }
                while(!stack.isEmpty()){
                    var c = stack.pop();
                    newQuery.addRecursiveQuery(c.iri(), c.query().cloneQuery());
                } 
    ...
    public static RecQuery shallowCopy(RecQuery query) {
        RecQueryShallowCopy copy = new RecQueryShallowCopy();
        query.visit(copy);
        RecQuery q2 = copy.newQuery;
        return q2;
    }
}
